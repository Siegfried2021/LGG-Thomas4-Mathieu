...
"""
Explanation: Base.metadata contains information about all the tables defined through SQLAlchemy models. create_all(engine) tells SQLAlchemy to generate the schema (i.e., the tables and columns) in the database using the engine, which is a connection to the database.
"""
...
"""
Explanation: When you call session.commit(), SQLAlchemy translates the queued operations (like adding my_new_book) into actual SQL commands and executes them in a single transaction. This is more efficient than executing each operation individually.
session.commit()
# 6. Close the session
session.close()
```
