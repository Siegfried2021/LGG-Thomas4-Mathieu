...
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    errors = exc.errors()
    missing_fields = [error['loc'][-1] for error in errors if error['type'] == 'value_error.missing']
    if missing_fields:
        return JSONResponse(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
            content=jsonable_encoder({"error": f"3 fields expected (salary, bonus, taxes). You forgot: {', '.join(missing_fields)}."}),
        )
    else:
        for error in errors:
            if error['type'] == 'type_error.integer':
                return JSONResponse(
                    status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
                    content=jsonable_encoder({"error": "expected numbers, got strings."}),
                )
    return JSONResponse(
        status_code=status.HTTP_422_UNPROCESSABLE_ENTITY,
